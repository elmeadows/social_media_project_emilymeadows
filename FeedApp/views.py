...
@login_required
def friends(request):
    #get the addmin_profile and user profile to create the first relationship
    ...
    all_profiles = Profile.objects.exclude(user=request.user).exclude(id__in=user_friends_profile).exclude(id__in=request_sent_profiles)
    ...
    context = {'user_friends_profile':user_friends_profile, 'user_relationships':user_relationships,
